...
async function addEmployee() {
    ...
    let managerResult = await db.promise().query(`SELECT e.id AS manager_id, CONCAT(e.first_name, ' ',  e.last_name) AS manager_name FROM employee AS m JOIN employee AS e ON m.manager_id = e.id WHERE m.manager_id IS NOT NULL`);
    ...
    db.query(`INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES ("${empFirstName}", "${empLastName}", ${roleId}, ${manId})`, function (err, result) {
        if (err) {
            console.log(err);
        }
    });
...
async function updateEmployee() {
    ...
    db.query(`UPDATE employee SET role_id = ${newEmpRoleId} WHERE id = ${empId}`, function (err, result) {
        if (err) {
            console.log(err);
        }
    });
